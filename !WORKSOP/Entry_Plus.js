...
var strat = {
  ...
  init: function() {
    ...
    //ATR
    this.addIndicator('atr', 'ATR', this.settings.ATR_Period);
  ...
  check: function(candle) {
    ...
    if (maFast < maSlow) {
      ...
      // ADX trend strength?
      if (adx < this.settings.ADX_low) rsi_low = rsi_low + this.BEAR_MOD_low;

      if (rsi < rsi_low && price <= bbands.lower) this.long(candle);
    ...
    // BULL TREND
      ...
      // ADX trend strength?
      if (adx < this.settings.ADX_low) rsi_low = rsi_low + this.BULL_MOD_low;

      if (rsi < rsi_low && price <= bbands.lower) this.long(candle);
      if (this.debug) this.lowHigh(rsi, 'bull');
    }

    this.checkShort(candle);
    // add adx low/high if debug
    if (this.debug) this.lowHigh(adx, 'adx');

  }, // check()

  checkShort: function(candle) {
    atr = this.indicators.atr.result;
    // Take Profit
    // if (candle.close >= this.entryPoint * (1 + (this.settings.takeAt / 100))) {
    if (candle.close >= this.entryPoint + (atr * (this.settings.takeAt / 100))) {
      this.short();
    }
    //Or Stoploss
    //if (candle.close < this.entryPoint * (1 - (this.settings.stopAt / 100))) {
    if (candle.close < this.entryPoint - (atr * (this.settings.stopAt / 100))) {
      this.short();
    }
